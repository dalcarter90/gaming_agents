...
class Game(ABC):
    """A turn-based game with perfect information.
    ...
    def key(self, state: State) -> Hashable:
        """A hashable summary of ``state`` for table-based agents.

        Two states sharing a key must be strategically interchangeable for the
        player to move, so a key may canonicalise (e.g. fold symmetries) but
        must never merge states that need different play.
        """
